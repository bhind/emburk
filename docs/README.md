# Documentation

Repository records are the authoritative project memory. GitHub Issues and the GitHub Project mirror these records for coordination.

## Product and Engineering

- [Product strategy](STRATEGY.md)
- [Roadmap](ROADMAP.md)
- [Architecture](ARCHITECTURE.md)
- [Compatibility](COMPATIBILITY.md)
- [Experimental native pipeline](NATIVE_PIPELINE.md)
- [Native MVP execution matrix](NATIVE_MVP_EXECUTION_MATRIX.md)
- [Performance evidence](PERFORMANCE.md)
- [GitHub Project operations](PROJECT_OPERATIONS.md)
- [Development](DEVELOPMENT.md)
  - Includes the bounded Qwen development-assistant CLI and IntelliJ entry points.
- [Qwen-assisted development](QWEN_ASSISTED_DEVELOPMENT.md)
  - Defines the Codex/Qwen responsibility split, data boundary, and verification loop.

## Governance

- [Governance](GOVERNANCE.md)
- [Workflow](WORKFLOW.md)
- [Kanban and Story Points](SPRINTS.md)
- [Current status](STATUS.md)
- [Traceable reimplementation and provenance](PROVENANCE.md)
- [Architecture decisions](decisions/README.md)
- [Provenance records](provenance/README.md)
- [Project log](log/README.md)

Actionable work is tracked canonically in [`TODO.md`](../TODO.md).
