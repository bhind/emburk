...
`3.5x` middle-row omission outcome. Stage B is not integrated; all remaining
unselected Float64 lexical values must continue to reject before publication.
This is not a general lexical, parser, or error-policy compatibility claim.
...
T-0071/S01, integrated through PR #127, adds selected end-to-end performance evidence against the same
...
T-0026/S01 is an integrated native-only observability slice. Its optional
configured run result JSON is an Emburk-owned experimental CLI contract, not an
observed Embulk format, diagnostic taxonomy, plugin result, or compatibility
surface. It changes no Native/Verified status.
