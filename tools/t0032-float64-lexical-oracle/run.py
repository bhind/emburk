#!/usr/bin/env python3
"""Capture three bounded Float64 lexical CSV observations from Embulk.

This original, evidence-only driver captures raw executable behavior.  It does
not decide native behavior or encode expected reference output.
"""
import hashlib
import json
import os
...
CASES = ("plain-decimals", "quoted-and-unquoted-decimal", "malformed-between-decimals")
TIMEOUT_SECONDS = 90
...
DRIVER_LABEL = "tools/t0032-float64-lexical-oracle/run.py"
...
def bounded_regular_bytes(path, root, limit):
    """Stat before reading a private regular file, then bind its raw bytes."""
    within(path, root)
    metadata = path.lstat()
...
def regular(path, root, limit=MAX_CASE_TREE_BYTES):
...
def reference(environment):
    ...
    contents = bounded_regular_bytes(path, path.parent, REFERENCE_BYTES)
...
def fixture(case):
    values = {
        "plain-decimals": b"ratio\n3.5\n-12.25\n",
        "quoted-and-unquoted-decimal": b'ratio\n3.5\n"3.5"\n',
        "malformed-between-decimals": b"ratio\n3.5\n3.5x\n42.0\n",
    }
    try:
        return values[case]
...
def run_case(case, capture_root, jar, java, version, environment_identity, run_uuid):
    ...
    environment = {"PATH": os.defpath, "JAVA_HOME": environment_identity["java_home"], "HOME": str(home), "TMPDIR": str(temporary), "EMBULK_HOME": str(home)}
    ...
    manifest = {"case": case, "run_uuid": run_uuid, "reference": {"sha256": REFERENCE_SHA256}, "environment": environment_identity,
                ...
                "config": canonical_text(root / "config.yml", root, MAX_CONFIG_BYTES), "input": canonical_text(root / "input.csv", root, MAX_INPUT_BYTES),
                "process": process, "outputs": checked_output(output)}
    manifest["case_tree"] = tree(root, MAX_CASE_TREE_BYTES)
...
def main():
    ...
    root = Path(tempfile.mkdtemp(prefix="emburk-t0032-s04-float64-lexical-capture-", dir="/private/tmp"))
    root.chmod(0o700)
    jar = root / "embulk.jar"; jar.write_bytes(jar_data); jar.chmod(0o400)
    run_uuid = str(uuid.uuid4())
    summary = {"run_uuid": run_uuid, "reference": {"sha256": REFERENCE_SHA256}, "reference_snapshot": regular(jar, root, REFERENCE_BYTES),
               "driver": {"path": DRIVER_LABEL, "sha256": digest(Path(__file__).read_bytes())}, "cases": [], "result": "captured"}
    try:
        for case in CASES:
            ...
            print("T0032_S04_CAPTURED=" + case, flush=True)
...
if __name__ == "__main__":
    try:
        ...
        print("T0032_S04_CAPTURE_ERROR=" + str(failure), file=sys.stderr)
        raise SystemExit(2)
