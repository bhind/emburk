...
/** Local-only T-0013/S07 output commit position observation probe. */
public final class T0013CommitPositionOutputPlugin implements OutputPlugin {
    private static final String INJECTED_MESSAGE = "t0013-output-commit-position-failure";
    private static final String CAPTURE_ID = UUID.randomUUID().toString();
    private static volatile int selectedCommitIndex = -1;
    private static volatile String selectionMode;
    ...
    public ConfigDiff transaction(
        ...
        if (taskCount < 3) {
            throw new InstrumentationError("output task count cannot distinguish positions");
        }
        selectCommit(fixture, taskCount);
        trace(
                fixture,
                "commit-selection",
                count,
                selectionMode,
                Integer.toString(selectedCommitIndex));
        TaskSource outputTaskSource = Exec.newTaskSource();
        trace(fixture, "control-run-before", count, columns);
        List<TaskReport> reports =
                runControl(fixture, "transaction-runtime-exception", control, outputTaskSource);
        if (reports == null) {
    ...
    public ConfigDiff resume(
            TaskSource taskSource, Schema schema, int taskCount, Control control) {
        String fixture = fixture();
        String count = Integer.toString(taskCount);
        String columns = Integer.toString(schema.getColumnCount());
        ...
        List<TaskReport> reports =
                runControl(fixture, "resume-runtime-exception", control, taskSource);
        if (reports == null) {
            throw new InstrumentationError("output resume control returned null reports");
        }
        trace(
                fixture,
                "resume-control-run-normal-return",
                count,
                columns,
                Integer.toString(reports.size()));
        ConfigDiff result = Exec.newConfigDiff();
    ...
    public TransactionalPageOutput open(
        ...
        TransactionalPageOutput result =
                new ObservedPageOutput(fixture, taskIndex, schema.getColumnCount());
        trace(fixture, "open-normal-return", index, columns);
        return result;
    }

    private static void selectCommit(String fixture, int taskCount) {
        switch (fixture) {
            case "normal":
            ...
            case "commit-middle":
                ...
                selectedCommitIndex = taskCount / 2;
                break;
            default:
                throw new InstrumentationError("unknown fixture mode");
        }
    }
    ...
    private static final class ObservedPageOutput implements TransactionalPageOutput {
        ...
        public TaskReport commit() {
            ...
            if (!"normal".equals(fixture) && taskIndex == selectedCommitIndex) {
                trace(fixture, "commit-injection-before", index, columns);
                InjectedCommitFailure failure =
                        new InjectedCommitFailure(INJECTED_MESSAGE);
                trace(
                        fixture,
    ...
    private static synchronized void trace(
            String fixture, String event, String... fields) {
        StringBuilder line =
                new StringBuilder("POSITIONOUTTRACE")
                        .append('|')
                        .append(fixture)
                        .append('|')
                        .append(CAPTURE_ID)
    ...
    /** Original exception used only by bounded selected commit fixtures. */
